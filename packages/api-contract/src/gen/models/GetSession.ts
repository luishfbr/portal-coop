/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/

import type { Session } from "./Session.ts";
import type { User } from "./User.ts";

/**
 * @description Success
*/
export type GetSession200 = {
    /**
     * @type object
    */
    session: Session;
    /**
     * @type object
    */
    user: User;
} | null;

/**
 * @description Bad Request. Usually due to missing parameters, or invalid parameters.
*/
export type GetSession400 = {
    /**
     * @type string
    */
    message: string;
};

/**
 * @description Unauthorized. Due to missing or invalid authentication.
*/
export type GetSession401 = {
    /**
     * @type string
    */
    message: string;
};

/**
 * @description Forbidden. You do not have permission to access this resource or to perform this action.
*/
export type GetSession403 = {
    /**
     * @type string | undefined
    */
    message?: string;
};

/**
 * @description Not Found. The requested resource was not found.
*/
export type GetSession404 = {
    /**
     * @type string | undefined
    */
    message?: string;
};

/**
 * @description Too Many Requests. You have exceeded the rate limit. Try again later.
*/
export type GetSession429 = {
    /**
     * @type string | undefined
    */
    message?: string;
};

/**
 * @description Internal Server Error. This is a problem with the server that you cannot fix.
*/
export type GetSession500 = {
    /**
     * @type string | undefined
    */
    message?: string;
};

export type GetSessionQueryResponse = GetSession200;

export type GetSessionQuery = {
    Response: GetSession200;
    Errors: GetSession400 | GetSession401 | GetSession403 | GetSession404 | GetSession429 | GetSession500;
};

export type GetSessionMutationRequest = object;

export type GetSessionMutationResponse = GetSession200;

export type GetSessionMutation = {
    Response: GetSession200;
    Request: GetSessionMutationRequest;
    Errors: GetSession400 | GetSession401 | GetSession403 | GetSession404 | GetSession429 | GetSession500;
};