...
function FloatingDelayGroup(props) {
  ...
  const delayRef = React7.useRef(delay);
  const initialDelayRef = React7.useRef(delay);
  const currentIdRef = React7.useRef(null);
  const currentContextRef = React7.useRef(null);
  const timeout = useTimeout();
  return (0, import_jsx_runtime2.jsx)(FloatingDelayGroupContext.Provider, {
    value: React7.useMemo(() => ({
      hasProvider: true,
      delayRef,
      initialDelayRef,
      currentIdRef,
...
function useDelayGroup(context, options = {
  ...
  const groupContext = React7.useContext(FloatingDelayGroupContext);
  const {
    currentIdRef,
    delayRef,
    timeoutMs,
  ...
  const [isInstantPhase, setIsInstantPhase] = React7.useState(false);
  useIsoLayoutEffect(() => {
    function unset() {
...
var FloatingPortal = React9.forwardRef(function FloatingPortal2(componentProps, forwardedRef) {
  ...
  return (0, import_jsx_runtime4.jsxs)(React9.Fragment, {
...
var useFloatingParentNodeId = () => React10.useContext(FloatingNodeContext)?.id || null;
...
function useClientPoint(context, props = {}) {
  ...
  const initialRef = React13.useRef(false);
  const cleanupListenerRef = React13.useRef(null);
  const [pointerType, setPointerType] = React13.useState();
  const [reactive, setReactive] = React13.useState([]);
  ...
  const addListener = React13.useCallback(() => {
...
function useDismiss(context, props = {}) {
  ...
  React14.useEffect(clearInsideReactTree, [outsidePress, clearInsideReactTree]);
...
function useFocus(context, props = {}) {
  ...
  const blockFocusRef = React17.useRef(false);
  const blockedReferenceRef = React17.useRef(null);
  const timeout = useTimeout();
  const keyboardModalityRef = React17.useRef(true);
  React17.useEffect(() => {
...
function useHoverFloatingInteraction(context, parameters = {}) {
  ...
  const closeWithDelay = React18.useCallback((event) => {
...
function useHoverReferenceInteraction(context, props = {}) {
  ...
  const tree = useFloatingTree(externalTree);
  const instance = useHoverInteractionSharedState(store);
  const isHoverCloseActiveRef = React19.useRef(false);
  ...
  React19.useEffect(() => cleanupMouseMoveHandler, [cleanupMouseMoveHandler]);
  React19.useEffect(() => {
    if (!enabled) {
      return void 0;
    }
...
function useInteractions(propsList = []) {
  ...
  const getReferenceProps = React20.useCallback(
    (userProps) => mergeProps(userProps, propsList, "reference"),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    referenceDeps
  );
  const getFloatingProps = React20.useCallback(
    (userProps) => mergeProps(userProps, propsList, "floating"),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    floatingDeps
  );
  const getItemProps = React20.useCallback(
    (userProps) => mergeProps(userProps, propsList, "item"),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    itemDeps
  );
  const getTriggerProps = React20.useCallback(
...
function useListNavigation(context, props) {
  ...
  const indexRef = React21.useRef(selectedIndex ?? -1);
  ...
  const previousMountedRef = React21.useRef(!!floatingElement);
  const previousOpenRef = React21.useRef(open);
  const forceSyncFocusRef = React21.useRef(false);
  const forceScrollIntoViewRef = React21.useRef(false);
  const cancelQueuedFocusRef = React21.useRef(null);
  ...
  const getParentOrientation = React21.useCallback(() => {
  ...
  const ariaActiveDescendantProp = React21.useMemo(() => {
...
function useRole(context, props = {}) {
  ...
  const floatingId = React22.useMemo(() => getFloatingFocusElement(floatingElement)?.id || defaultFloatingId, [floatingElement, defaultFloatingId]);
  ...
  const item = React22.useCallback(({
    active,
    selected
  }) => {
    const commonProps = {
...
function useTypeahead(context, props) {
  ...
  const timeout = useTimeout();
  const stringRef = React23.useRef("");
  const prevIndexRef = React23.useRef(selectedIndex ?? activeIndex ?? -1);
  const matchIndexRef = React23.useRef(null);
  useIsoLayoutEffect(() => {
...
function useFloating2(options = {}) {
  ...
  const [localFloatingElement, setLocalFloatingElement] = React24.useState(null);
...
//# sourceMappingURL=chunk-AV3QAMMW.js.map
